...
def results():
    ...
    cursor.execute("SELECT COUNT(*) FROM patients WHERE denture_type = 'lower' AND denture_type IS NOT NULL")
    lower_denture_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM patients WHERE denture_type = 'upper' AND denture_type IS NOT NULL")
    ...
    plt.title('Hány alsó fogsor? Hány felső fogsor?')
    plt.ylabel('Szám')
    ...
    # ROC Analysis for MAI
    fpr, tpr, thresholds, roc_auc, optimal_idx, optimal_threshold_mai = [0], [0], [0], 0, 0, 0

    if len(final_mai_scores) == len(init_mai_scores):
        mai_score_difference = np.array(final_mai_scores) - np.array(init_mai_scores)
        cursor.execute("SELECT perceived_change FROM patients WHERE perceived_change IS NOT NULL")
        reported_improvement = cursor.fetchall()
        reported_improvement = [row[0] for row in reported_improvement]
        reported_improvement = [1 if answer in ['Kicsit javult', 'Sokat javult'] else 0 for answer in reported_improvement]
        
        # Check if there are at least two unique values in reported_improvement
        ...
        print("MAI scores are not equal")
        roc_img_mai = None
        diff_img_mai = None

    # ROC Analysis for OHIP
    if len(ohip_final_scores) == len(ohip_init_scores):
        ohip_score_difference = np.array(ohip_final_scores) - np.array(ohip_init_scores)
        # Check if there are at least two unique values in reported_improvement for OHIP
        if len(np.unique(reported_improvement)) > 1:
        ...
        print("OHIP scores are not equal")
        roc_img_ohip = None
        diff_img_ohip = None  
    ...
    if len(gohai_final_scores) == len(gohai_init_scores):
        gohai_score_difference = np.array(gohai_final_scores) - np.array(gohai_init_scores)
        # Check if there are at least two unique values in reported_improvement for GOHAI
        if len(np.unique(reported_improvement)) > 1:
        ...
        print("GOHAI scores are not equal")
        roc_img_gohai = None
        diff_img_gohai = None
